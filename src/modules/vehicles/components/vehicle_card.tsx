...
export function VehicleCard({ vehicle }: { vehicle: Vehicle }) {
  return <article className="vehicle-card">
    ...
    <div className="card-content">
      <div className="card-heading"><div><p className="eyebrow">Opção para consulta</p><h3>{vehicle.model}</h3></div><span className={`status ${vehicle.acceptsInterest ? "status-available" : "status-unavailable"}`}>{vehicle.availabilityLabel}</span></div>
      ...
      <div className="card-actions"><Link href={`/veiculos/${vehicle.id}`} className="button secondary">Ver detalhes</Link>{vehicle.acceptsInterest ? <Link href={`/interesse?vehicle=${vehicle.id}`} className="button primary">Tenho interesse</Link> : <span className="button button-disabled" aria-disabled="true">Interesse indisponível</span>}</div>
    </div>
  </article>;
}
