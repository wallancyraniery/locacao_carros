...
A migration 0010 acrescenta somente USAGE em `reservation_submission_private` e EXECUTE na assinatura estreita `submit(uuid, uuid, date, date, text, text, text, text, boolean, text, boolean, text, text)` para a runtime. Não altera permissões nas tabelas nem as credenciais. O novo repository usa uma chamada autocommit em READ COMMITTED e retorna recibo sem PII; `rental_leads` e tabelas privadas de reservas continuam sem SELECT direto. Este contrato novo foi preparado para validação local; não implica aplicação ou comprovação remota da 0010.
