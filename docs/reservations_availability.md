...
`requested` e lista de espera não bloqueiam períodos, mesmo quando sobrepostos entre si ou a um bloco existente. Somente blocos `active` tornam o período indisponível. O status editorial/operacional atual de `vehicles` e a consulta do catálogo não são convertidos em agenda nesta entrega.
