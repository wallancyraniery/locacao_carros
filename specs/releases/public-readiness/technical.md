...
O formulário preserva o honeypot e possui integração Turnstile server-side implementada no PR-001. O desenvolvimento usa um modo sintético proibido em produção; chaves, hostname público e ativação externa permanecem para o PR-004. Cada formulário recebe uma identidade opaca de operação, preservada nos retries e persistida com unicidade. Cada token Turnstile recebe outro UUID, preservado somente nos retries da mesma validação; o servidor deriva do par operação + tentativa o `idempotency_key` da Siteverify. Um token novo recebe uma nova chave de validação sem trocar a identidade do formulário, e trocar somente a operação não permite reaproveitar um token consumido. Isso mantém o retry técnico idempotente sem deduplicar pessoas. A continuidade opcional pelo WhatsApp e o runbook humano estão implementados localmente no PR-003, mas o número oficial, os responsáveis, o mecanismo administrativo e a validação prática permanecem gates operacionais. O PR-003 continua `todo` até esses gates serem atendidos.
